...
>
> 🔄 **2026-09-21 AI 를 붙이지 않는다**(결정 #28) — Re:Read 서버의 AI 프록시 · 사용량 원장 · Anthropic 경로(§6)는 폐기했고 **기록으로 남긴다.**
> 지식 본문이 기기를 떠나는 경로가 없어졌다. 수익은 **광고 + 광고 제거**(결정 #29)라 광고 요청이 Google 로 간다.
> Re:Read 서버에 남은 일은 (v1.1) 백업 금고뿐이고 v1.1 에 구글 로그인과 함께 세운다(결정 #30).
> v1.0 의 문의 · 공지는 common_server 에 **기기 subject**(로그인 없음 · `POST /api/v1/devices` · LinkMemo · Idea Repository 승계)로 붙는다.
...
| Re:Read 서버 `server/` 생성 | ❌ | 🔄 할 일은 (v1.1) 백업 금고뿐 · v1.1 에 로그인과 함께(결정 #30) |
| ~~AI 프록시 라우트~~ | 🚫 | 결정 #28(2026-09-21) |
| 백업 금고 | ❌ | v1.1 |
| 커뮤니티 | 🚫 | 3차. 설계만 §7 |
...
## 6. ~~🔴 AI 처리 경로 — 지나가되 저장하지 않는다~~ 🚫 폐기 (결정 #28 · 2026-09-21)

> AI 를 붙이지 않아 이 경로는 생기지 않는다. 아래는 기록이다. 🟢 백업 고지 규율(§6.2 "저장하지 않습니다" ≠ "읽지 못합니다")은 백업 금고를 두면 그대로 쓴다.
