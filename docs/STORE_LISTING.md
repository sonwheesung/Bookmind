...
>
> 🔄 **2026-09-21**: AI 를 붙이지 않고(결정 #28) 수익은 **광고 + 광고 제거**다(결정 #29). 지금 배포본에는 광고가 없어서 아래 답은 **여전히 참이다.**
> 🔴 **광고 SDK 를 넣는 빌드 전에 바꿀 답**: 광고 여부(예) · 광고 ID 선언(사용) · 데이터 보안(광고 식별자 · 수집 있음) · 디지털 상품 구매(광고 제거 · 예) · 등록정보 문구의 *"광고도 없습니다"* · 처리방침.
>
...
| 다운로드 뒤 접근하는 외부 콘텐츠(영상·상품·생성형 AI 등) | 아니요 | 🔄 결정 #28 로 AI 를 안 붙여 바뀔 일이 없다 · ~~Phase 8 이 붙으면 "예"~~ |
| 연령 제한 상품(담배·술·화기·도박) 홍보·판매 | 아니요 | |
| 정확한 위치를 다른 사용자와 공유 | 아니요 | 위치 권한이 없다 |
| 디지털 상품 구매 | 아니요 | 🔴 **광고 제거 구매가 붙으면 "예"로 바뀐다**(결정 #29 · ~~구독~~) |
...
⚠ **이 선언은 배포본 기준이라 기능이 붙으면 반드시 바뀐다.** 문의가 붙으면 기기 ID 와 문의 내용이(결정 #30), v1.1 에 로그인이 붙으면 신원이, 광고가 붙으면
광고 식별자와 광고 SDK 가 생긴다(결정 #29). 그때 **선언과 처리방침을 같이** 고친다. 🔄 ~~AI 가 붙으면 원문이 프록시를 지나간다~~ 는 결정 #28 로 없어졌다.
...
⚠ **Phase 10(광고 SDK · `expo-iap` · 결정 #31)을 붙이는 날 이 답을 다시 본다.** 서드파티 SDK 가 광고 ID 를 가져오면 "예"여야 한다(콘솔 안내 원문).

🔴 **2026-09-21 광고 코드가 들어왔다**(`MONETIZATION_SYSTEM.md` §A). 위 표는 vc3 기준이고 **광고를 켠 첫 AAB 부터 거짓이 된다.**
🟢 **vc11 은 아직 참이다.** 비공개 테스트 동안 광고를 꺼 두고 `AD_ID` 를 `blockedPermissions` 로 막았다(§A.6.1). vc11 매니페스트에 `AD_ID` 가 **없는 것**을 manifest-merger 보고서와 `check:aab` 로 확인했다. 그래서 콘솔을 안 건드리고 올렸다.
광고를 켠 AAB 를 올리기 **전에** 콘솔에서 넷을 바꾼다.

| 무엇 | 지금 | 바꿀 값 |
|---|---|---|
| 광고 ID 선언 | 아니요 | **예** · 용도 광고 |
| 광고 포함 | 아니요 | **예** |
| 데이터 보안 | 수집 · 공유 없음 | 기기 또는 기타 ID(광고 식별자) · 앱 활동(광고 상호작용) · 🔴 Google 로 **공유** · 구매 기록(Play 결제) |
| 처리방침 URL 내용 | v1.1 | **v1.2**(`docs/legal/PRIVACY.*.md` · 광고 · 광고 제거) |

`check:aab` 는 `AD_ID` 를 금지에서 허용으로 옮겼다. 첫 광고 AAB 에서 실제 권한 집합을 재서 허용 목록을 확정한다.
