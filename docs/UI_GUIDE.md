...
| 공통 부품 **16개**(§2) | 🔨 2026-09-13. 파일은 16개 다 있다(기존 9 + 새 7). 🔴 **화면은 아직 새 부품을 안 쓴다**(§7.5) |
| 공통 헬퍼(§3) | 🔨 2026-09-13. 함수는 있고 화면이 아직 안 부른다(§7.5) |
| 가드 `npm run check:ui`(§6) | ❌ 설계만 있다. `verify` 에 아직 없다 |
...
**16개** (세는 법: `ls components/*.tsx | wc -l` · 가드가 생기면 `check:ui` ⑦이 이 표와 파일을 대조한다)
...
| `formatDate(iso, lang)` | `lib/format.ts` | 날짜 표시. 로케일은 UI 언어를 넘긴다 |
...
- 🔴 **새 부품을 만들면 §2 표에 적는 것까지가 완료다.** 가드가 생기면 `check:ui` ⑦이 빠진 줄을 잡는다.
...
## 6. 가드 `npm run check:ui` (❌ 설계 · 아직 없다)
...
### 7.5 ⏸ 2026-09-13 에 멈춘 자리

사용자 지시 *"여기까지 하고 정리해줄래"* 로 멈췄다. **앱 동작은 하나도 안 바뀌었다.**

| 무엇 | 상태 |
|---|---|
| 이 문서 · README 색인 | ✅ |
| 새 부품 7개 · 헬퍼(`lib/format` · `lib/confirm` · `lib/set` · `WEEKDAY_KEYS` · `splitTagInput` · `coverInk`) | ✅ 파일만 |
| 기존 부품 5개(`BookCover` · `BookSelect` · `Header` · `WeekRow` · `Field`)를 새 부품으로 | ❌ |
| 화면 16개를 새 부품·헬퍼로 (§7.1 의 처리 열) | ❌ |
| 가드 `check:ui` · `check:knowledge` 에 `splitTagInput` 경계 추가 | ❌ |

🔴 **§7.2 의 화면 변화는 이전이 끝난 뒤에야 생긴다.** 지금 앱에는 없다.
