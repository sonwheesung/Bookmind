...
| 공통 부품 **23개**(§2) | ✅ 2026-09-14. 기존 9 + 새 7 + 연령 게이트 층(`AgeGate`) 1 · 🔄 같은 날 `IconButton` · `MonthCalendar` 을 더하고 `BookCover` 를 지웠다 · 🔄 2026-09-15 차트 부품 셋(`BarChart` · `ActivityGrid` · `RankBars`)을 더했다 · 🔄 2026-09-18 `Select`(설정의 언어)를 더했다 · 🔄 2026-09-21 `AdBanner`(하단 배너)를 더했다. 화면 16개와 기존 부품 5개가 이전을 마쳤다(§7.5) |
| 공통 헬퍼(§3) | ✅ 2026-09-14 |
...
**23개** (세는 법: `ls components/*.tsx | wc -l` · `check:ui` ⑦이 이 표와 파일을 대조한다)

| 부품 | 쓰는 자리 | 규칙 |
|---|---|---|
| `Screen` | 모든 화면의 바깥 | 🔴 화면에서 SafeAreaView·ScrollView 를 직접 쓰지 않는다. 키보드 가림도 여기서 처리한다 · 위 여백 = 인셋 + `spacing.lg`(2026-09-14) · 탭 화면은 `tab` 을 넘겨 아래 인셋을 탭 바에 맡긴다 · `footer` 는 스크롤 밖 맨 아래 자리(지금은 광고 배너만 · 키보드가 뜨면 내린다) |
...
| `AdBanner` | 하단 고정 배너(홈 · 문장 · 책 · 복습 끝) | 🔴 `Screen` 의 `footer` 자리에만 둔다(스크롤 밖 · 탭 화면은 탭 바 바로 위) · 광고 제거를 샀거나 준비 전이면 아무것도 안 그린다 · 둘 곳은 `AD_PLACEMENTS` 네 화면뿐이고 `check:ads` 가 잰다(`MONETIZATION_SYSTEM.md` §A.3) |
