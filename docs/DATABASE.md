...
> 🔄 **2026-09-21 휴면 표**(결정 #28). AI 를 붙이지 않아 늘 비어 있다. 🔴 **지우지 않는다** —
> 마이그레이션은 덧붙이기만 하고(§1), 백업 형식이 이 표를 들고 있어 빼면 옛 백업 파일을 못 읽는다(`BACKUP_SYSTEM.md`).
...
> 🔄 **2026-09-21 휴면 표**(결정 #28). `ai_analyses` 와 같은 이유로 비어 있고 지우지 않는다.
