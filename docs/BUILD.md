...
③ 콘솔에서 게시     ⏸ 아직 안 했다
④ 🔴 권한 회수(5→4) ✅ 새로고침해서 4 로 남아 있는 것까지 확인
```

🔴 **②가 끝나자마자 ④를 했다.** ③(게시)은 콘솔에서 사람이 누르는 일이라 권한과 무관하다.
권한을 켜 둔 시간을 줄이는 것이 규율이고, 게시를 기다리며 켜 두면 그 시간이 길어진다.

🟢 **§5.13 의 함정을 두 번 다 피했다.** 켤 때도 끌 때도 `find` 로 ref 를 잡아 눌렀고,
누른 뒤 **확대해서 체크 상태를 읽었다.** 실제로 켤 때 다이얼로그가 클릭 직후 스크롤했고,
좌표를 썼다면 그 순간 **바로 위 줄(`프로덕션으로 출시`)** 을 눌렀을 자리다.
확대 판정에서 `프로덕션으로 출시`가 양쪽 다 **꺼짐**인 것을 확인했다.
