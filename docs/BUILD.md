...
## 6.6 vc7 — 위 오른쪽 아이콘 · 표지 제거 · 문장 책별 · 실천 달력 (2026-09-14)

사용자 지시 *"우선 이정도 수정하고 빌드해서 업로드 진행해"*. 화면별 수정 목록을 전부 반영한 뒤의 빌드다(수정마다 빌드하지 않는다).
...
🔴 **화면을 확인하지 않고 올린다**(에뮬레이터 작업 중단). 설치 뒤 **아이콘 탭 · 헤더 아이콘 · 문장 책별 보기 · 실천 달력 · 홈 버튼과 구분선**을 본다.

### 실측 (2026-09-14)

| | vc6 | vc7 | |
|---|---:|---:|---|
| 빌드 시간 | 5m 53s | **약 7m 28s**(스크립트 시작 17:42:08 → AAB 17:49:36 · verify · prebuild 포함) | 🔴 Gradle 로그가 끝까지 안 남았다(아래) |
| AAB 크기 | 74,635,039 B | **77,946,796 B** | **+3.3MB.** `react-native-svg` 네이티브 라이브러리 네 ABI(항목 8개) |
| 권한 수 | 28 | 🟢 **28** | SVG 가 권한을 안 늘렸다 · `check:aab` 허용 목록 일치 |
| versionCode · version | 6 · 0.6.0 | **7 · 0.7.0** | `android/app/build.gradle` |
| 서명 | 업로드 키 | **업로드 키** | SHA1 `44:0E:B4:48:…:C4:1C` 일치 |
| OTA 배선 | | `expo-channel-name` · `u.expo.dev/8785afeb-…` | 병합 매니페스트 |
| 보관 | | `D:\builds\Bookmind\reread-vc7.aab` | sha256 `d5adc16e…81f1` 원본과 일치 |
| 빌드 뒤 java 프로세스 | 0 | **0** | |
| 뒷정리 | 2.0G → 421K | **2.2G → 421K** | |

🔴 **빌드 셸이 PC 메모리 부족으로 강제 종료됐다**(여유 1.7GB / 32GB · WSL 7.9GB · 다른 에뮬레이터 1.6GB 가 함께 떠 있었다).
Gradle 자식 프로세스는 살아서 끝까지 갔지만 **스크립트의 뒤 단계(서명 · OTA · 권한 게이트)는 안 돌았다.** 그래서 AAB 의 시각 · 버전 · 서명 · 권한을 손으로 다시 쟀다(위 표). 기다리는 감시 작업도 한 번 같은 이유로 죽었다.
★ **셸이 죽은 것과 빌드가 실패한 것은 다르다.** 로그 끝(`BUILD_EXIT`)이 없으면 성공도 실패도 말할 수 없고, 산출물을 직접 연다.

✅ **업로드 · `alpha` 트랙 · 2026-09-14** — API 로 다시 읽었다: `alpha` = `0.7.0` · `completed` · versionCode `7`(vc6 을 대체). `production` 은 **비어 있다.**
⚠ 출시 노트는 이번에도 비어 있다(`eas submit`).

## 6.7 vc8 — 통계 차트 · 책 상태 · 문구 검수 · 버전 이름 규칙 (2026-09-15)

사용자 지시 *"오늘 작업한거 비공개 트랙에 올려줘"*.
...
🔴 **빌드는 분리된 프로세스로 돌린다.** vc7 에서 백그라운드 셸이 메모리 부족으로 강제 종료돼 스크립트 뒤 단계가 안 돌았다(§6.6). 이번에는 셸과 떨어진 프로세스가 로그 파일에 쓰고, 끝났는지는 로그의 `BUILD_EXIT` 줄로 판정한다.
🔴 **화면을 확인하지 않고 올린다**(에뮬레이터 작업 중단). 설치 뒤 **통계 차트 · 책 상세 상태 · 바뀐 문구**를 본다. ⚠ 활동한 날이 7일 미만이면 차트는 원래 안 뜬다(`STATS_SYSTEM.md` §4.2).

### 실측 (2026-09-15)

| | vc7 | vc8 | |
|---|---:|---:|---|
| 빌드 시간 | 약 7m 28s(셸 강제 종료) | **스크립트 전체 13m 55s** · Gradle `12m 59s` | 🟢 분리 프로세스라 **끝까지 돌았다**(`BUILD_EXIT=0`). 서명 · OTA · 권한 게이트가 스크립트 안에서 돌았다 |
| AAB 크기 | 77,946,796 B | **77,955,617 B** | +8.8KB. 통계 차트 · 부품 셋 · 문구 |
| 권한 수 | 28 | 🟢 **28** | 허용 목록 일치 |
| versionCode · version | 7 · 0.7.0 | **8 · 0.1.8** | `android/app/build.gradle` · 결정 #27 |
| runtimeVersion | `1.0.0` | 🔴 **`native-2`** | `strings.xml` 과 AAB `resources.pb` 에서 확인 · `check:ota` ⑦ 네이티브 반영까지 통과 |
| 서명 | 업로드 키 | **업로드 키** | SHA1 `44:0E:B4:48:…:C4:1C` 일치 |
| OTA 배선 | | `expo-channel-name` · `u.expo.dev/8785afeb-…` | 병합 매니페스트 |
| 보관 | | `D:\builds\Bookmind\reread-vc8.aab` | sha256 `fcbd140a…67a0` 원본과 일치 |
| 빌드 뒤 java 프로세스 | 0 | 끝난 직후 2개 → **곧 0** | JVM 이 내려가는 중이었다. 종료 명령은 쓰지 않았다 |

✅ **업로드 · `alpha` 트랙 · 2026-09-15** — API 로 다시 읽었다: `alpha` = `0.1.8` · `completed` · versionCode `8`(vc7 을 대체). `production` 은 **비어 있다.**
⚠ 출시 노트는 이번에도 비어 있다(`eas submit`).

## 6.8 vc9 — 관리자 수정사항 #1~#4 (2026-09-16)

사용자 지시 *"비공개 테스트에 수정된거 안 올라간거 같은데"* → *"vc9 빌드해서 올려줘"*.
🔴 **수정사항 네 건은 vc8 뒤에 한 작업이라 트랙에 없다.** 테스트 기간에는 OTA 를 안 쓰므로(결정 #23) AAB 로만 간다.
...
🟢 **이번엔 화면을 보고 올린다** — 네 건 다 에뮬레이터(`reread` 5574 · Expo Go · ko)에서 확인했다(`UI_GUIDE.md` §2.1 · `PRACTICE_SYSTEM.md` §3 · `KNOWLEDGE_SYSTEM.md` §3.2.1).
⏸ 실기기와 다크 모드는 아직 못 봤다. 설치 뒤 그 둘을 본다.
🔴 빌드는 §6.7 과 같이 **분리된 프로세스**로 돌리고 `BUILD_EXIT` 줄로 판정한다.

### 실측 (2026-09-16)

| | vc8 | vc9 | |
|---|---:|---:|---|
| 빌드 시간 | Gradle 12m 59s | Gradle **9m 54s** | 684 태스크(408 실행 · 248 캐시 · 28 최신) |
| AAB 크기 | 77,955,617 B | **77,956,594 B** | +977 B. JS 만 바뀌었다 |
| 권한 수 | 28 | 🟢 **28** | `check:aab` 허용 목록 일치 |
| versionCode · version | 8 · 0.1.8 | **9 · 0.1.9** | `android/app/build.gradle` |
| runtimeVersion | `native-2` | **`native-2` 그대로** | `strings.xml` · 네이티브 변경 없음 |
| 서명 | 업로드 키 | **업로드 키** | SHA1 `44:0E:B4:48:…:C4:1C` 일치 |
| 보관 | | `D:\builds\Bookmind\reread-vc9.aab` | sha256 `8a3cc83e…ff15` 원본과 일치 |

✅ **업로드 · `alpha` 트랙 · 2026-09-16** — API 로 다시 읽었다: `alpha` = `0.1.9` · `completed` · versionCode `9`(vc8 을 대체). `production` 은 **비어 있다.**
⚠ 출시 노트는 이번에도 비어 있다(`eas submit`).

### 🔴🔧 빌드가 **안 돌았는데 돌고 있는 줄 알았다** (2026-09-16)

분리 프로세스에 로그 경로를 `C:/Users/...` 로 줬다. **bash 는 그것을 상대 경로로 읽는다** — 리디렉션이 실패하면 그 명령은 아예 안 돈다.
그런데 같은 시각 다른 프로젝트(`basketball_story`)가 `assembleRelease` 를 돌리고 있어서, 프로세스 목록의 `java` · `aapt2` 를 보고 **우리 빌드가 도는 줄 알았다.**

→ 규칙 둘:
- 분리 프로세스의 로그 경로는 **bash 형식(`/c/...`)** 으로 준다. 띄운 직후 **로그 파일이 생겼는지**로 시작을 판정한다(프로세스 목록이 아니라).
- 프로세스로 판정할 때는 **명령줄에 프로젝트 경로가 들어 있는지**까지 본다. 이 PC 는 형제 프로젝트가 같이 돈다.

⚠ 그리고 **이 PC 는 메모리가 빠듯하다**(그때 31.9GB 중 여유 2.1GB · `basketball_story` 의 Next 개발 서버 하나가 11.8GB). 감시용 백그라운드 셸이 두 번 메모리 부족으로 종료됐다. 빌드 자체는 분리 프로세스라 살아남았다.

## 6.9 vc10 — 실천 시작일 이전 체크 · 언어 셀렉트 · 차트 첫날부터 (2026-09-18)
...
⏸ **화면을 확인하지 않고 올린다**(사용자 지시 · 에뮬레이터는 다른 앱 작업으로 꺼 둔 상태). 설치 뒤 **실천 달력의 시작일 이전 체크 · 설정 언어 셀렉트 · 통계 세로 막대**를 본다.
🔴 §6.8 의 규칙대로 로그 경로는 bash 형식 · 시작은 로그 파일 생성으로 판정 · 다른 프로젝트 빌드와 겹치지 않게 띄운다.

### 실측 (2026-09-18)

| | vc9 | vc10 | |
|---|---:|---:|---|
| 빌드 시간 | Gradle 9m 54s | Gradle **9m 16s** | 띄운 지 1초 만에 로그 파일이 생겨 시작을 확인했다(§6.8 규칙) · 그때 다른 프로젝트 빌드 없음(대기 중인 데몬만) · 여유 메모리 4.0GB |
| AAB 크기 | 77,956,594 B | **77,958,124 B** | +1.5KB. `Select` 부품 · 실천 · 통계 |
| 권한 수 | 28 | 🟢 **28** | `check:aab` 허용 목록 일치 |
| versionCode · version | 9 · 0.1.9 | **10 · 0.1.10** | `android/app/build.gradle` |
| runtimeVersion | `native-2` | **`native-2` 그대로** | `strings.xml` |
| 서명 | 업로드 키 | **업로드 키** | SHA1 `44:0E:B4:48:…:C4:1C` 일치 |
| 보관 | | `D:\builds\Bookmind\reread-vc10.aab` | sha256 `5d494bf6…1285` 원본과 일치 |

✅ **업로드 · `alpha` 트랙 · 2026-09-18** — API 로 다시 읽었다: `alpha` = `0.1.10` · `completed` · versionCode `10`(vc9 을 대체). `production` 은 **비어 있다.**
⚠ 출시 노트는 이번에도 비어 있다(`eas submit`).
...
| | vc10 | vc11 | |
|---|---:|---:|---|
| 빌드 시간 | Gradle 9m 16s | Gradle **9m 44s** | 다른 프로젝트 빌드 없음(대기 데몬만) · 여유 메모리 10.1GB |
| AAB 크기 | 77,958,124 B | **85,324,453 B** | +7.4MB. 광고 SDK(Play services ads · WorkManager) · Play 결제 |
| 권한 수 | 28 | **33** | + 결제 1 · 광고 서비스 3 · 🔴 `FOREGROUND_SERVICE` 1(아래) · `AD_ID` 는 **없다**(manifest-merger 보고서가 세 라이브러리의 선언을 REJECTED 로 적었다) |
| versionCode · version | 10 · 0.1.10 | **11 · 0.1.11** | |
| runtimeVersion | `native-2` | **`native-3`** | `strings.xml` |
| 서명 | 업로드 키 | **업로드 키** | SHA1 `44:0E:B4:48:…:C4:1C` 일치 |
| 보관 | | `D:\builds\Bookmind\reread-vc11.aab` | sha256 앞 16자 `f9e4a35a4158eadb` 원본과 일치 |

🔴 **첫 결과는 `BUILD_EXIT=1` 이었다.** 빌드는 성공했고 빌드 뒤 `check:aab` 가 모르는 권한 `FOREGROUND_SERVICE` 하나로 멈췄다.
출처는 광고 SDK 가 끌어오는 `androidx.work:work-runtime:2.7.0` 이다. 일반 권한이고 유형별 FGS 권한이 아니라 허용 목록에 근거와 함께 넣고 **같은 AAB 를 다시 잰 뒤** 올렸다(재빌드 없음).
★ 허용 목록의 "예상 목록"(§A.7)이 한 칸 틀렸다. 예상은 넷이었고 실측은 다섯이다. 가드가 막았기 때문에 알았다.
