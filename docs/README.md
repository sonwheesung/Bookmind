...
| 문서 | 범위 | 상태 |
|---|---|---|
| [`../CLAUDE.md`](../CLAUDE.md) | 설계 정본 — 기둥 7개 · MVP 범위 · 서버 경계 · 스택 · 결정 로그 **27건**(세는 법: `grep -c "^\*\*#" CLAUDE.md`) · 미결정 **5건**(세는 법: `grep -c "^\| [A-H] \|" CLAUDE.md` — 닫힌 것은 `~~F~~` 로 취소선 처리되어 자동으로 빠진다) | ✅ 2026-09-08 |
...
| 책 등록 · 목록 · 상세 · **수정** | ✅ | Phase 2(2026-09-09). 파생 집계 4종 · 제목·저자 수정 · 🔴 삭제해도 지식은 남는다 |
...
| ~~`common/DEV_ALLOCATION.md` Re:Read 행~~ | ✅ | Metro 8091 · 서버 3500 · Supabase 248xx · AVD `reread` 5574. ⚠ 2026-09-10 대조 전까지 ❌ 로 남아 있었다 |
...
**`check:review` 변이 주입 실측(2026-09-09 · 8종 전부 발화)**

| 변이 | 결과 |
|---|---|
| 생각 단서 우선순위 제거 | **exit 2** — SELF-TEST 가 "갈래를 못 가른다"로 먼저 잡는다 ✅ |
| 앞 글자 경계 `>` → `>=` | `경계(=12자)에서 단서를 줬다 — 다 보여주게 된다` ✅ |
| 이모지를 코드 유닛으로 자르기 | `이모지 원문에서 글자 수가 어긋난다` ✅ |
| 공백 단서 허용(trim 제거) | `공백뿐인 생각을 단서로 썼다` ✅ |
| 큐 정렬 제거 | `오래된 순이 아니다 — 밀린 것이 계속 밀린다` ✅ |
| 상한 무시 | **exit 2** ✅ |
| 새 카드가 저장 당일에 뜬다 | `새 카드가 저장 시점보다 앞에 예약됐다` ✅ |
| 로컬 자정 → UTC 자정 | **exit 2** — 같은 날인데 하루의 끝이 다르다 ✅ |
...
#### 🔴 화면에서 고친 것을 확인할 때는 **앱을 껐다 켠다** (2026-09-09 실측)

`adb shell am start` 로 딥링크를 다시 열어도 **이미 떠 있는 앱은 옛 번들을 그대로 돈다.**
그날 빈 원문 저장 잠금을 고치고 확인했는데 **버튼이 여전히 활성**이었다 —
코드는 맞았고 화면이 옛 번들이었다. `am force-stop` 후 다시 열어서야 반영됐다.

⚠ **이걸 모르면 "고쳤는데 안 고쳐졌다"로 오진하고, 더 나쁘게는 옛 번들이 우연히 통과해
"고쳐졌다"고 잘못 판정한다.** 화면 판정 앞에는 항상 강제 종료를 넣는다.
...
- 🔴 **스크립트로 파일을 고칠 때는 인코딩을 파일 열기 *전에* 끝낸다** (2026-09-08 실사고, §3.1).
  ...
  → 🟢 **2026-09-09 해소: `C:\project\common` 이 git 저장소가 됐다**(공통서버 세션이 `git init`,
  근거는 우리가 낸 0바이트 사고다). **`.bak` 을 새로 만들지 않는다** — git 이 그 역할을 한다.
  ⚠ 다만 `BUSINESS_INFO.md` 와 `business-docs/` 는 여전히 **추적 제외**다(민감정보). 그쪽은 조심히 다룬다.
...
- 🟢 **`DOC_SYSTEM.md` §6.1 의 "고치기 전에 사본을 뜬다"가 그대로 값을 했다.**
  당시 `common` 은 git 레포가 아니라 사본이 유일한 복구 수단이었다(20,408 바이트 완전 복구).
  → 🟢 **그 사고가 근거가 되어 2026-09-09 에 `common` 이 git 저장소가 됐다.** 이제 복구 수단은 git 이다.
- → 처방은 위 규칙 두 개다: **인코딩 먼저 · 원자적 교체**.

#### 🔴 고치는 도구가 값을 바꾸면 안 된다 (2026-09-09 · 공통서버 세션 실측, 승계)

같은 자리(`DEV_ALLOCATION.md` 의 `D:\emulators\reread`)에서 **CR 손상이 또 났다.**
어제 한 번 고친 자리라, "자연 발생률이 낮지 않다"가 실증됐다.

🔴 **그런데 고치는 과정에서 더 나쁜 일이 날 뻔했다.** 파이썬 **텍스트 모드**로 읽으면 lone CR 이 개행으로 바뀐다:

```
읽기 전   D:\emulators<0x0d>eread
텍스트 모드로 읽고 다시 쓰기 → D:\emulators<개행>eread
```

**제어문자는 사라지고 검사는 통과하는데 줄이 쪼개진다** — 손상이 "고쳐진 것처럼" 보이면서 내용만 조용히 틀린다.
→ **읽기도 쓰기도 바이트로 한다**(`newline=''` 로 열거나 `rb`/`wb`). 우리 `check:chars` 는 **바이트로 읽으므로**
   탐지 쪽은 안전하다. 위험한 것은 **고치는 쪽**이고, 이 항목은 그래서 있다.

🔴 **그리고 이 절을 쓰다가 같은 결함을 또 냈다**(2026-09-09, 같은 세션). 위 경로를 문서에 적는 순간
`\r` 이 CR 로 접혔고, `check:chars` 가 커밋 전에 잡았다 — offset 19197.
★ **세 번째 재발이고 전부 같은 문자열이다.** 이 경로는 이제 손으로 적지 않는다 — 적어야 하면 백슬래시를
`chr(92)` 로 만들어 조립한다(위 규칙 그대로). 가드가 없었으면 "CR 조심하라"고 적은 문서가 CR 을 품고 나갔다.
