...
- `"native-2"` 는 **네이티브 세대 번호**이고 `version`(테스트 동안 `0.1.<versionCode>`)과 **아무 관계가 없다.** 🔴 **앱 버전 모양(`x.y.z`)으로 쓰지 않는다**(§3.1 · 🔄 2026-09-15 전까지는 `"1.0.0"` 이었다).
- 🚫 **둘이 달라 보인다고 맞추지 마라.** 맞추는 순간 기존 빌드가 전부 OTA 고아가 된다.
  가드가 이 실수를 FAIL 로 막는다(§9).
- 범프하는 때는 **네이티브 모듈이 바뀔 때뿐**이다. `version` 을 올릴 때가 아니다.

### 3.1 🔴 `1.0.0` → `native-2` (2026-09-15 · 결정 #27)

| 무엇 | 값 |
|---|---|
| 🔴 범프를 두 번 빠뜨렸다 | vc2 에 ML Kit, vc7 에 `react-native-svg` 가 들어갔는데 `1.0.0` 그대로였다. 지금 OTA 를 쏘면 vc6 이하 기기에 그림 모듈 없이 아이콘 코드가 가서 켜자마자 죽는다 |
| 🔴 출시 이름과 겹친다 | 정식 출시 이름이 `1.0.0` 이다(결정 #27). 같아지는 순간 ⑤ 가 막고, 막지 않으면 짝이 꼬인다 |
| 새 이름 | **`native-2`**. 앱 버전 모양이 아니라 앞으로도 안 겹친다. 네이티브가 바뀔 때마다 `native-3` · `native-4` 로 올린다 |
| 적용 | vc8 부터. vc7 이하는 앞으로 OTA 를 못 받고 Play 업데이트로만 옮겨 온다. 2026-09-11 에 발행한 runtime `1.0.0` 페이로드(§7.2)도 vc8 이후에는 안 닿는다 |
| 가드 | ⑤ 가 `x.y.z` 모양을 막는다. ⑦ 은 `strings.xml` 값을 **꺾쇠까지 붙여** 대조한다(`native-1` 이 `native-10` 안에 들어맞지 않게). ⑧ 이 테스트 기간 이름 `0.1.<versionCode>` 를 잰다 |

★ "네이티브가 바뀌면 범프한다"는 규칙은 이 문서에 있었는데 **빌드 기록에는 없었다.** AAB 를 일곱 번 구우면서 `BUILD.md` 표에 *"runtimeVersion `1.0.0` 고정"* 이라고만 적었다.
→ 앞으로 빌드 기록의 runtimeVersion 행에는 **"지난 빌드 뒤 네이티브 모듈이 바뀌었나"** 를 함께 적는다.
...
| 🔴 ⑤ runtimeVersion | **고정 문자열**인가(정책 객체 금지) · **`version` 과 같지 않은가**(같으면 고아가 된다) · 🔄 **앱 버전 모양(`x.y.z`)이 아닌가**(결정 #27) |
| 🔴 ⑥ 버전 오염 | 앱 소스가 `expoConfig.version`·`versionCode` 를 직접 읽지 않나(§6) |
| ⑦ 네이티브 반영 | 빌드된 AAB 매니페스트에 `EXPO_UPDATE_URL` 과 채널 헤더가 실제로 들어갔나(§10) |
| ⑧ 버전 이름 | 비공개 테스트 동안 `version` 이 `0.1.<versionCode>` 인가 · `1.0.0` 이상은 건너뛴다(결정 #27) |
